...
async function initNurseDashboard() {
    ...
    document.getElementById('register-patient-form').addEventListener('submit', async (e) => {
        ...
        if (res.ok) {
            alert('Patient registered! Please allocate a bed and doctor from the pending list.');
    ...
    if (allocForm) {
        allocForm.addEventListener('submit', async (e) => {
            e.preventDefault();
            const patientId = allocForm.dataset.patientId;
            const bedId = document.getElementById('alloc-bed').value;
            const docId = document.getElementById('alloc-doctor').value;
            ...
            if (res.ok) {
                ...
                const data = await res.json();
                alert('Failed to allocate: ' + (data.error || 'Unknown error'));
            }
        });
    }
}
...
async function initDoctorDashboard() {
    ...
    if (searchInput) {
        searchInput.addEventListener('input', (e) => {
            loadDoctorPatients(e.target.value);
        });
    }
...
async function loadPendingAllocationList() {
    ...
    if (statPending) statPending.textContent = patients.length;
...
async function openAllocationModal(patientId) {
    // Add cache-busting timestamp to prevent stale bed data
    const bedsRes = await fetch('/api/beds?t=' + new Date().getTime());
    ...
    if (availableBeds.length === 0) {
        alert('No more beds available!');
        return;
    }

    const bedSelect = document.getElementById('alloc-bed');
    const docSelect = document.getElementById('alloc-doctor');
    if (!bedSelect || !docSelect) return;

    bedSelect.innerHTML = availableBeds.map(b => `<option value="${b.id}">${b.ward} - ${b.bed_number}</option>`).join('');
    docSelect.innerHTML = doctors.map(d => `<option value="${d.id}">Dr. ${d.name} (${d.specialization})</option>`).join('');
...
async function loadAdminPatients(search = '') {
    ...
    const patients = await res.json();
    const tableBody = document.querySelector('#admin-patient-table tbody');
    if (!tableBody) return;
    tableBody.innerHTML = '';
